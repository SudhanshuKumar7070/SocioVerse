...
const createTweet = AsyncHandler(async (req, res) => {
  ...
  if (!TextContent)
    throw new ApiError(400, "can't create empty tweet required");
  const userId = req.user._id;
  if (!userId) throw new ApiError(400, "unauthorised access");

  let contentImageCloudUrl;
  const tweetImageLocalPath = req.files["contentImage"]
    ? req.files["contentImage"][0]
    : null;
  if (tweetImageLocalPath) {
    const contentImageCloudPath = await uploadProfileImage(tweetImageLocalPath);
    if (!contentImageCloudPath)
      throw new ApiError(500, "error in uploading content image to cloudinary");
    contentImageCloudUrl = contentImageCloudPath?.url;
  }
  let tweetVideoUrl;
  const tweetVideoLocalPath = req.files["contentVideo"]
    ? req.files["contentVideo"][0]
    : null;
  if (tweetVideoLocalPath) {
    ...
    if (!videoCloudPath)
      throw new ApiError(
        500,
        "something went wrong in uploading video to cloudinary",
      );
...
const deleteTweet = AsyncHandler(async (req, res) => {
  ...
  if (tweet.userId.toString() !== userId.toString())
    throw new ApiError(400, "you are not owner of the tweet");

  const deletedTweet = await Tweet.findByIdAndDelete(tweetId);
  if (!deletedTweet)
    throw new ApiError(400, "unable to delete tweet at the moment");
...
const editTweet = AsyncHandler(async (req, res) => {
  ...
  if (tweet.userId.toString() !== userId.toString())
    throw new ApiError(
      400,
      "you are not owner of the tweet , prohibited action",
    );
  // uodate tweet
  ...
  if (!updatedTweet)
    throw new ApiError(400, "unable to update tweet at the moment");
  return res
    .status(200)
...
const editTweetUrl = AsyncHandler(async (req, res) => {
  ...
  if (!newImageLocalPath)
    throw new ApiError(400, " image is not available locally");
  const cloudinaryPath = await uploadProfileImage(newImageLocalPath);
  if (!cloudinaryPath.url)
    throw new ApiError(
      500,
      "something went wrong in uplaoding image to cloudinary",
    );
  const updatedImage = await Tweet.findByIdAndUpdate(
  ...
  if (!updatedImage)
    throw new ApiError(500, "something went wrong  in updating image of tweet");
  return res
    .status(200)
    .json(new ApiResponse(200, updatedImage, "data updated successfully"));
});
...
const getTweetByTweetId = AsyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, userTweet[0], "Tweet fetch success"));
});
...
const getAllTweets = AsyncHandler(async (req, res) => {
  ...
  const limitNo = parseInt(limit) > 0 ? parseInt(limit) : 10000;
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, tweet, "tweet fetched sucsessfully"));
});
...
const getIndividualsTweets = AsyncHandler(async (req, res) => {
  ...
  if (!tweets || tweets.length === 0)
    throw new ApiError(404, " something went wrong in finding tweets");
  return res
    .status(200)
    .json(new ApiResponse(200, tweets, "tweets fetched successfully"));
});
